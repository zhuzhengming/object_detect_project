...
void IMU_Decode(void){
    ...
    for (int i = 0; i < IMU_BAG_LENGHTH; i++) {
        IMU_Bag_t tempBag = *((IMU_Bag_t*) (void*) (&(imu_revbag[i])));
...
void YOLO_Decode(void){
    ...
    {
        YOLO_Bag_t tempBag = *((YOLO_Bag_t*)(void*)(&(yolo_revbag[i])));
        if(tempBag.head[0] == YOLO_HEADER_HIGH && tempBag[1] == YOLO_HEADER_LOW)
        {
