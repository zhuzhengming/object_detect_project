...
#define IMU_BAG_LENGHTH (IMU_HEAD_LENGTH+ IMU_PAYLOAD_LENGTH)
...
#define YOLO_BAG_LENGTH (YOLO_HEAD_LENGTH+YOLO_PAYLOAD_LENGTH)


typedef union {
    uint8_t raw[IMU_BAG_LENGHTH ];
    struct {
        union {
...
typedef union {
    uint8_t raw[YOLO_HEAD_LENGTH + YOLO_PAYLOAD_LENGTH ];
    struct {
